...
export type TableProps = {
    ...
    fetchList: (t: Tab, pg: number, limit?: number) => void,
...
const Table = (props: TableProps) => {
    const { tab, meta, list, loading, page, fetchList, deleting, resolving, resolve, remove, setPage } = props
                ...
                <div style={{ ...card(), padding: 0, overflow: "hidden" }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                            <button
                                onClick={() => fetchList(tab, page)}
                                style={{ ...btn("ghost"), padding: "6px 10px", fontSize: 12 }}
                                disabled={loading}
                            >
                                <RefreshCw size={13} style={{ animation: loading ? "spin 1s linear infinite" : "none" }} />
                            </button>
                            <LimitSelect value={limit} onChange={(val) => { onLimitChange(val); fetchList(tab, 1, val) }} />
                        </div>
                    </div>
                                    ...
                                    {list.map((s, i) => (
                                        <tr
                                            ...
                                            style={{
                                                ...
                                                background: selectedItem?.id === s.id
                                                    ? `${C.p}0f`
                                                    : i % 2 === 0 ? C.white : "#fafcfb",
                                                opacity: deleting === s.id ? 0.4 : 1,
                                                transition: "background 0.15s, opacity 0.2s",
                                                cursor: "pointer",
                                            }}
                                            onMouseEnter={e => {
                                                if (selectedItem?.id !== s.id)
                                                    (e.currentTarget as HTMLElement).style.background = C.bg
                                            }}
                                            onMouseLeave={e => {
                                                if (selectedItem?.id !== s.id)
                                                    (e.currentTarget as HTMLElement).style.background = i % 2 === 0 ? C.white : "#fafcfb"
                                            }}
                                        >
                                            <td style={{ padding: "12px 16px", fontWeight: 500, color: C.t, fontSize: 13, whiteSpace: "nowrap" }}>
                                                {s.name}
                                            </td>
                                            ...
                                            {tab === "feedback" && (
                                                <td style={{ padding: "12px 16px", whiteSpace: "nowrap" }}>
                                                    <Badge color={statusColor((s as Feedback).status)}>
                                                        {statusLabel((s as Feedback).status)}
                                                    </Badge>
                                                </td>
                                            )}
                                                    ...
                                                    {tab === "feedback" && (
                                                        <button
                                                            ...
                                                            style={{
                                                                ...
                                                                fontSize: 12,
                                                                color: C.green,
                                                                opacity: (s as Feedback).status === "resolved" ? 0.4 : 1,
                                                            }}
                                                        >
                                                            {resolving === s.id
