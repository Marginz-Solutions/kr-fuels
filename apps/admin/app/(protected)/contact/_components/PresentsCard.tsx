...
const cardHeader = (iconBg: string, iconColor: string, icon: React.ReactNode, title: string, subtitle: string) => (
  <div style={{
    ...
    <div>
      ...
      <div style={{ fontSize: 11, color: C.p, marginTop: 1 }}>{subtitle}</div>
    </div>
  </div>
)
        ...
        <div style={{ padding: "12px 20px" }}>
          {SOCIAL_PLATFORMS.map((platform, i) => (
            <div key={platform.key} style={{
              ...
              <span style={{ fontSize: 12, fontWeight: 500, color: C.p, minWidth: 72, flexShrink: 0 }}>
                {platform.label}
              </span>
              <input
