...
const TestimonialCard: FC<TestimonialCardProps> = ({ testimonial }) => {
  ...
  return (
    ...
    >
      ...
      <svg
        className="absolute top-5 right-5 w-9 h-9 text-brand/20 group-hover:text-brand transition-colors duration-300 pointer-events-none"
        aria-hidden="true"
        viewBox="0 0 40 40"
        fill="currentColor"
      >
      ...
      <blockquote className="flex-1 mb-5">
        <p className="text-mutedfg text-sm leading-relaxed line-clamp-5">
          &ldquo;{message}&rdquo;
        </p>
      </blockquote>

      {/* ── Teal accent rule ──────────────────────────────────────────── */}
      <div className="w-8 h-0.5 bg-brand mb-4 rounded-full" />
      ...
      <footer className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          {byline && (
            <div className="text-xs text-mutedfg mt-0.5 truncate" title={byline}>
              {byline}
            </div>
          )}
        </div>
